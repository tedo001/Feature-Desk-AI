...
export default function FloatingMonitor() {
  ...
  const [pos, setPos] = useState(() => ({
    x: 16,
    y: typeof window !== "undefined" ? Math.max(16, window.innerHeight - 440) : 80,
  }));
  ...
  if (collapsed) {
    return (
      <button
        ...
        className="flex items-center gap-2 rounded-full bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-lg hover:bg-indigo-700 cursor-move"
        title="Open AI monitoring (drag to move)"
      >
        <Camera className="h-4 w-4" /> AI Monitor
      </button>
    );
  }
  ...
  return (
    <div
      style={{ position: "fixed", left: pos.x, top: pos.y, zIndex: 50, width: 340 }}
      className="max-w-[92vw] shadow-2xl rounded-2xl"
    >
      {/* drag handle */}
      <div
        onPointerDown={onPointerDown}
        onPointerMove={onPointerMove}
        onPointerUp={onPointerUp}
        className="flex items-center justify-between rounded-t-2xl bg-indigo-600 px-3 py-1.5 cursor-move select-none"
      >
        <span className="flex items-center gap-1.5 text-xs font-medium text-white">
          <GripHorizontal className="h-4 w-4" /> Drag to move
        </span>
        <button
